export default {
  ...
  async run(ctx: any) {
    if (["delmodprefix", "deletemodprefix"].includes(ctx.cmdName)) {
      ...
      return ctx.reply("✅ Prefijo de moderación eliminado. Se usarán los prefijos globales.");
    }

    const value = String(ctx.args?.[0] ?? "").trim();
    if (value && (value.length > 3 || /\s/.test(value))) {
      return ctx.reply("❌ El prefijo debe tener entre 1 y 3 caracteres y no puede contener espacios.");
    }
