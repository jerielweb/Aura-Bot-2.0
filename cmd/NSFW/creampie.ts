...
run: async ({ msg, text, sender, db, reply, react }: any) => {
  await react("🥛");
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=creampie&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("le hizo un creampie a")} \`${targetName}\` 🥛🔥`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere un creampie")} 🥛`;
      }
