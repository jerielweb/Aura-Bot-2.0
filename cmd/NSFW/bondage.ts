...
export default {
	...
	async run({ args, reply, react, msg, from, sender, text, db, isGroup }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/nsfw/interaction?inter=bondage&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("ató a")} \`${targetName}\` ⛓️`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere atar a alguien")} ⛓️`;
			}
