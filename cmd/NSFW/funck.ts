...
export default {
  name: ["fuck", "coger", "follar"],
  description: "Reacción NSFW de fuck.",
  category: "nsfw",
run: async ({ msg, text, sender, db, reply, react }: any) => {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=fuck&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("se follo a")} \`${targetName}\` 🍑💥`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere follar")} 🍑`;
      }
