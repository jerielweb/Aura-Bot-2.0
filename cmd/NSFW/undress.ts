...
export default {
  name: ["undress", "desnudar"],
  description: "Reacción NSFW de desnudar.",
  category: "nsfw",
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=undress&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("desvistió a")} \`${targetName}\` 👀🔥`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere desnudarse")} 👀`;
      }
