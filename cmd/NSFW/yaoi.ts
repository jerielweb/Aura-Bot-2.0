...
export default {
  ...
  category: "nsfw",
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=yaoi&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("en modo yaoi con")} \`${targetName}\` 💙🔥`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("está en modo yaoi")} 💙`;
      }
