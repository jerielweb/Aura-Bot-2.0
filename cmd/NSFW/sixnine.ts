...
run: async ({ msg, text, sender, db, reply, react }: any) => {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=sixnine&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("hizo un 69 con")} \`${targetName}\` 🔄🔥`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere  aser un 69")} 🔄`;
      }
