...
run: async ({ msg, text, sender, db, reply, react }: any) => {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=grope&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("manoseó a")} \`${targetName}\` 🙈🔥`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere manosear")} 🙈`;
      }
