...
export default {
  ...
  async run({ args, reply, react, msg, from, sender, text, db, isGroup }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=blowjob&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("le hizo una mamada a")} \`${targetName}\` 🍆💦`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere una mamada")} 🍆`;
      }
