...
export default {
  name: ["cum"],
  description: "Reacción NSFW de acabada.",
  category: "nsfw",
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=cum&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("se vino en")} \`${targetName}\` 💦`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("se vino")} 💦`;
      }
