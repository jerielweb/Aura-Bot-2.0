...
export default {
  name: ["cumshot", "venirse"],
  description: "Reacción NSFW de cumshot.",
  category: "nsfw",
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=cumshot&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("se vino encima de ")} \`${targetName}\` 💦😳`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("se vino encima")} 💦`;
      }
