...
export default {
  name: ["spank", "nalgada"],
  description: "Reacción NSFW de nalgada.",
  category: "nsfw",
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=spank&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("le dio una nalgada a")} \`${targetName}\` 🍑`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere una nalgada")} 🍑`;
      }
