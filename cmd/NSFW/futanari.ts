...
export default {
  ...
  async run({ args, reply, react, msg, from, sender, text, db, isGroup }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/nsfw/interaction?inter=futanari&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("tuvo accion futanari  con")} \`${targetName}\` 🍆🎀`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("está en modo futanari")} 🍆`;
      }
