...
export default {
  ...
  async run(ctx: any) {
    ...
    const reason = (ctx.args || []).filter((arg: string) => !arg.includes("@")).join(" ").trim();
    if (!reason) return ctx.reply({ text: `╭〔 ⚠️ ${fytBold("AURA REED")} 〕⬣\n┃ ${fytBold("FALTA MOTIVO")}\n╰━━━━━━━━━━━━⬣\n\n┃ > Debes especificar una razón.\n┃ > Ejemplo: ${ctx.usedPrefix || "."}warn @usuario Insultos\n\n╰〔 ⚡ ${fytBold("SYSTEM INFO")} 〕⬣` });
