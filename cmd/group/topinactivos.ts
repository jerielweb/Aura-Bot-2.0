...
export default {
  ...
  adminOnly: true,
  async run(ctx: any) {
    const activity = Array.isArray(ctx.db.getGroup(ctx.from).topMsgUsers)
      ? ctx.db.getGroup(ctx.from).topMsgUsers
      : [];
    const counts = new Map<string, number>();
    for (const user of activity) {
      const count = Number(user.count || 0);
      for (const identity of [user.jid, user.lid, user.phoneNumber].filter(
        Boolean,
      )) {
    ...
    text += users
      .map(
        (user: any) => `┃ ➪ @${user.id.split("@")[0]} › ${user.count} mensajes`,
      )
      .join("\n");
