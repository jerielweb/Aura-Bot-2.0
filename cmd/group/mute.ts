...
export default {
  ...
  async run(ctx: any) {
    ...
    const isAdmin = ctx.groupMeta?.participants?.some((participant: any) => participant.id === target && participant.admin);
    if (isAdmin) return ctx.reply({ text: `╭〔 ❌ ${fytBold("AURA REED")} 〕⬣\n┃ ${fytBold("ACCIÓN PROHIBIDA")}\n╰━━━━━━━━━━━━⬣\n\n┃ > No puedes silenciar a un administrador.\n\n╰〔 ⚡ ${fytBold("SYSTEM ALERT")} 〕⬣` });
