...
export default {
  ...
  async run({ args, reply, react }: any) {
    ...
    try {
      ...
      const param = isUrl ? `url=${encodeURIComponent(query.split("?")[0])}` : `query=${encodeURIComponent(query)}`;
      const response = await requestJson(`${api}${endpoint}?${param}&key=${DL_CONFIG.alya.API_KEY}`);
      const song = response?.data;
      const download = typeof song?.dl === "string" ? song.dl : song?.dl?.mp3;
      if (!response?.status || !download) throw new Error("No se pudo obtener el audio.");
      ...
      await reply({ document: { url: download }, mimetype: "audio/mpeg", fileName: `${safeFileName(title, "spotify")}.mp3` });
      await react("✅");
    } catch (error: any) { await react("❌"); return reply({ text: `❌ Error: ${error?.message || "No se pudo descargar Spotify."}` }); }
  },
};
