...
class SoundCloudHttpError extends Error {
	...
	constructor(statusCode: number, body = "") {
		super(`SoundCloud respondió HTTP ${statusCode}${body ? `: ${body.slice(0, 160)}` : ""}`);
...
async function fetchText(url: string, init: any = {}): Promise<string> {
	...
	if (response.statusCode < 200 || response.statusCode >= 300) {
		throw new SoundCloudHttpError(response.statusCode, await response.body.text());
	}
...
async function getClientId(): Promise<string> {
	if (cachedClientId && Date.now() - cachedAt < 60 * 60 * 1000) return cachedClientId;
...
async function resolveTrack(query: string, clientId: string): Promise<any> {
	...
	if (!query.includes("soundcloud.com/")) {
		...
		if (!result) throw new Error("No se encontraron resultados para tu búsqueda.");
	...
	if (!track || track.kind !== "track") throw new Error("No se encontró un track válido.");
	return track;
}
	...
	async run({ args, reply, react }: any) {
		...
		try {
			...
			const transcoding = track.media?.transcodings?.find(
				(item: any) => item.format?.mime_type === "audio/mpeg" && item.format?.protocol === "progressive",
			);
			...
			const safeTitle = String(track.title || "soundcloud")
				.replace(/[<>:"/\\|?*]/g, "")
				.slice(0, 100) || "soundcloud";
			const streamData = await requestJson(streamUrl, 30000);
			const audioUrl = streamData?.url;
			if (!audioUrl) throw new Error("SoundCloud no devolvió una URL de audio válida.");
			...
			await react("❌");
			await reply({ text: `❌ Error: ${error?.message || "No se pudo descargar el audio."}` });
		}
	},
};
