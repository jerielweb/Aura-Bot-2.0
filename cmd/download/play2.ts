...
async function searchVideo(query: string): Promise<string> {
  const data = await requestJson(`${DL_CONFIG.alya.BASE_URL.replace(/\/+$/, "")}/search/yt?query=${encodeURIComponent(query)}&key=${DL_CONFIG.alya.API_KEY}`);
  if (!data?.status || !data.result?.[0]?.url) throw new Error("No se encontró ningún video.");
...
export default {
  ...
  async run({ args, reply, react }: any) {
    ...
    try {
      ...
      const data = await requestJson(`${API}/dl/ytv?url=${encodeURIComponent(url)}&quality=1080&apikey=${KEY}`, 60000);
      if (!data?.status || !data?.datos?.url) throw new Error("La API no pudo procesar el video.");
      ...
      if (data.miniatura) await reply({ image: { url: data.miniatura }, caption });
      const file = await downloadBuffer(data.datos.url);
      await reply({ video: file, mimetype: "video/mp4", fileName: `${safeFileName(title, "youtube")}.mp4` });
      await react("✅");
    } catch (error: any) {
      await react("❌");
