...
export default {
  ...
  async run({ args, reply, react }: any) {
    ...
    if (!url) return reply("⚠️ Proporciona un enlace de Facebook o Facebook Reels.");
    await react("⏳");
    try {
      const data = await requestJson(`${API}/dl/facebook?url=${encodeURIComponent(url)}&key=${DL_CONFIG.alya.API_KEY}`, 60000);
      const item = data?.data?.[0] || data?.result?.[0] || data?.data;
      const videoUrl = typeof item === "string" ? item : item?.url || item?.hd || item?.sd;
      ...
      await reply({ video: file, mimetype: "video/mp4", fileName: "facebook.mp4", caption });
      await react("✅");
    } catch (error: any) {
      await react("❌");
