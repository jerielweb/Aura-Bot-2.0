...
async function text(url: string): Promise<string> {
  const response = await request(url, { headers: { "User-Agent": "Mozilla/5.0" }, signal: AbortSignal.timeout(30000) });
...
async function clientId(): Promise<string> {
  ...
  for (const script of scripts.slice(-5)) {
    ...
    if (match?.[2]) { cachedClientId = match[2]; cachedAt = Date.now(); return cachedClientId; }
  }
...
export default {
  ...
  async run({ args, reply, react }: any) {
    ...
    if (!query) return reply("⚠️ Proporciona una búsqueda o enlace de SoundCloud.");
    await react("🎵");
    try {
      const id = await clientId();
      let url = query;
      if (!query.includes("soundcloud.com/")) {
        const search = await json(`https://api-v2.soundcloud.com/search/tracks?q=${encodeURIComponent(query)}&client_id=${id}&limit=1`);
        url = search.collection?.[0]?.permalink_url || "";
      }
      if (!url) throw new Error("No se encontró ningún track.");
      const track = await json(`https://api-v2.soundcloud.com/resolve?url=${encodeURIComponent(url)}&client_id=${id}`);
      const transcoding = track.media?.transcodings?.find((item: any) => item.format?.mime_type === "audio/mpeg" && item.format?.protocol === "progressive");
      if (!transcoding) throw new Error("Este track no tiene un stream MP3 descargable.");
      ...
      await reply({ document: { url: stream.url }, mimetype: "audio/mpeg", fileName: `${safeFileName(title, "soundcloud")}.mp3` });
      await react("✅");
    } catch (error: any) { await react("❌"); return reply({ text: `❌ Error: ${error?.message || "No se pudo descargar SoundCloud."}` }); }
  },
};
