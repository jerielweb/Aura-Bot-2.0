...
export default {
  ...
  async run({ args, reply, react }: any) {
    ...
    if (!query) return reply("⚠️ Ingresa el nombre de una canción o un enlace de Spotify.");
    await react("🎵");
    try {
      const isUrl = query.includes("open.spotify.com/");
      const endpoint = isUrl ? "/dl/spotify" : "/dl/spotifyplay";
      const parameter = isUrl ? `url=${encodeURIComponent(query.split("?")[0])}` : `query=${encodeURIComponent(query)}`;
      const response = await requestJson(`${API}${endpoint}?${parameter}&key=${KEY}`);
      const song = response?.data;
      const downloadUrl = typeof song?.dl === "string" ? song.dl : song?.dl?.mp3;
      if (!response?.status || !song || !downloadUrl) throw new Error("No se pudo obtener el audio.");
      ...
      if (song.coverHd || song.cover) await reply({ image: { url: song.coverHd || song.cover }, caption });
      else await reply({ text: caption });
      await reply({ audio: { url: downloadUrl }, mimetype: "audio/mpeg", fileName: `${safeFileName(title, "spotify")}.mp3` });
      await react("✅");
    } catch (error: any) {
      await react("❌");
      return reply({ text: `❌ Error: ${error?.message || "No se pudo descargar la canción."}` });
    }
  },
};
