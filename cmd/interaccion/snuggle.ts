...
export default {
	name: ["snuggle", "arrimar"],
	description: "Envía una reacción de arrimarse.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=snuggle&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("se arrimó a")} \`${targetName}\` 🥺❤️`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere arrimarse")} 🥺`;
			}
