...
export default {
	name: ["laugh", "reir", "risa"],
	description: "Envía una reacción de risa.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=laugh&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("se rió de")} \`${targetName}\` 😂`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("se está riendo")} 😂`;
			}
