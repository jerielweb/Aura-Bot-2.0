...
export default {
	name: ["smug", "presumir", "creído"],
	description: "Envía una reacción de presunción.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=smug&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("presume ante")} \`${targetName}\` 😏✨`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("está presumiendo")} 😏`;
			}
