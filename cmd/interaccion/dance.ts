...
export default {
	name: ["dance", "bailar"],
	description: "Envía una reacción de baile.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=dance&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("bailó con")} \`${targetName}\` 💃`;
