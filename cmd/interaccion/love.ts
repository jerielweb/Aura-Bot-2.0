...
export default {
	name: ["love", "amar", "amor"],
	description: "Envía una reacción de amor.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=love&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("ama a")} \`${targetName}\` ❤️💕`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("está enamorado")} ❤️`;
			}
