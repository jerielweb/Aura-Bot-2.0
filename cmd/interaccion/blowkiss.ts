...
export default {
	name: ["blowkiss", "besovolado"],
	description: "Envía una reacción de beso volado.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=blowkiss&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("le mandó un beso volado a")} \`${targetName}\` 😘💨`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere mandar un beso volado")} 😘`;
			}
