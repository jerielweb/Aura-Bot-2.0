...
export default {
  name: ["killall", "matartodos"],
  description: "Envía una reacción de asesinato.",
  ...
  async run({ args, reply, react, msg, from, sender, text, db }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/sfw/interaction?inter=kill&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("mató a todos con ")} \`${targetName}\` 💀`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("mato a todos ")} 💀`;
      }
