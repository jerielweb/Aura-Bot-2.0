...
export default {
	name: ["handhold", "mano", "tomardelamano"],
	description: "Envía una reacción de tomar de la mano.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=handhold&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("tomó de la mano a")} \`${targetName}\` 🤝❤️`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere que le tomen de la mano")} 🤝`;
			}
