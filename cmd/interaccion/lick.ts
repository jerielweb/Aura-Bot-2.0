...
export default {
  name: ["lick", "lamer"],
  description: "Envía una reacción de lamer.",
  ...
  async run({ args, reply, react, msg, from, sender, text, db }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/sfw/interaction?inter=lick&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("lamió a")} \`${targetName}\` 👅`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere lamer")} 👅`;
      }
