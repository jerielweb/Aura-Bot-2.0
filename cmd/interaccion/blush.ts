...
export default {
  name: ["blush", "sonrojar"],
  description: "Envía una reacción de sonrojarse.",
  ...
  async run({ args, reply, react, msg, from, sender, text, db }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/sfw/interaction?inter=blush&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("se sonrojó por")} \`${targetName}\` 😳`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("está sonrojado")} 😳`;
      }
