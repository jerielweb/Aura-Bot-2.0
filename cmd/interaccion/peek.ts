...
export default {
  name: ["peek", "espiar"],
  description: "Envía una reacción de espiar.",
  ...
  async run({ args, reply, react, msg, from, sender, text, db }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/sfw/interaction?inter=peek&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("espió a")} \`${targetName}\` 👀`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere espiar")} 👀`;
      }
