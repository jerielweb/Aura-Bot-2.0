...
export default {
	name: ["highfive", "choca", "chocar"],
	description: "Envía una reacción de chocar los cinco.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=highfive&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("chocó los cinco con")} \`${targetName}\` 🙌`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere chocar los cinco")} 🙌`;
			}
