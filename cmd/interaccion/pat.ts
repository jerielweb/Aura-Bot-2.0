...
export default {
	name: ["pat", "acariciar", "patpat"],
	description: "Envía una reacción de caricia.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=pat&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("acarició a")} \`${targetName}\` 🤚✨`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere una caricia")} 🤚✨`;
			}
