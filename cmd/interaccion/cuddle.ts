...
export default {
	name: ["cuddle", "acurrucar", "mimo"],
	description: "Envía una reacción de acurrucarse.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=cuddle&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("acurrucó a")} \`${targetName}\` 🥰`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere acurrucarse")} 🥰`;
			}
