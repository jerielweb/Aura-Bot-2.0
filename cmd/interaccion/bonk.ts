...
export default {
	name: ["bonk", "tonto"],
	description: "Envía una reacción de bonk.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=bonk&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("le dio un bonk a")} \`${targetName}\` 🔨`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere dar un bonk")} 🔨`;
			}
