...
export default {
  name: ["hug", "abrazo"],
  description: "Envía una reacción de abrazo.",
  ...
  async run({ args, reply, react, msg, from, sender, text, db }: any) {
    ...
    try {
      ...
      const apiUrl = `${baseUrl}/sfw/interaction?inter=hug&key=${DL_CONFIG.alya.API_KEY}`;
      ...
      if (targetJid) {
        ...
        const targetName =
          ...
          targetUser?.username ||
          targetJid.split("@")[0];
        caption = `\`${senderName}\` ${fytBold("abrazó a")} \`${targetName}\` 🤗`;
        mentions = [sender, targetJid];
      } else {
        caption = `\`${senderName}\` ${fytBold("quiere un abrazo")} 🤗`;
      }
