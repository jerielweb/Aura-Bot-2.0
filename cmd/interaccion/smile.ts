...
export default {
	name: ["smile", "sonreir", "sonrisa"],
	description: "Envía una reacción de sonrisa.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=smile&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("le sonrió a")} \`${targetName}\` 😊`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("está sonriendo")} 😊`;
			}
