...
export default {
	name: ["tickle", "cosquillas"],
	description: "Envía una reacción de cosquillas.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=tickle&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("hizo cosquillas a")} \`${targetName}\` 🤭`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere hacer cosquillas")} 🤭`;
			}
