...
export default {
	name: ["angry", "enojado", "bravo"],
	description: "Envía una reacción de enojo.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=angry&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("está enojado con")} \`${targetName}\` 😡🔥`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("está enojad@")} 😡`;
			}
