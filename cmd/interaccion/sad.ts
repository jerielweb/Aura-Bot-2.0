...
export default {
	name: ["sad", "triste"],
	description: "Envía una reacción de tristeza.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=sad&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("está triste por")} \`${targetName}\` 😔`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("está triste")} 😔`;
			}
