...
export default {
	name: ["cringe", "cringeo", "pena"],
	description: "Envía una reacción de cringe.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=cringe&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("hizo cringe a")} \`${targetName}\` 😬`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("sintió cringe")} 😬`;
			}
