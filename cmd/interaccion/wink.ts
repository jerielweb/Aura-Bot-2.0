...
export default {
	name: ["wink", "guino", "guinar"],
	description: "Envía una reacción de guiño.",
	...
	async run({ args, reply, react, msg, from, sender, text, db }: any) {
		...
		try {
			...
			const apiUrl = `${baseUrl}/sfw/interaction?inter=wink&key=${DL_CONFIG.alya.API_KEY}`;
			...
			if (targetJid) {
				...
				caption = `\`${senderName}\` ${fytBold("le guiñó el ojo a")} \`${targetName}\` 😉`;
				mentions = [sender, targetJid];
			} else {
				caption = `\`${senderName}\` ${fytBold("quiere guiñar el ojo")} 😉`;
			}
