...
import { economyUser, formatCoins, saveEconomy } from "../../core/economyRuntime.ts";
...
export default {
  ...
  async run(ctx: any) {
    ...
    if (user.lastPpt && now - user.lastPpt < cooldown) return ctx.reply(`⏳ Espera *${cooldownText(cooldown - (now - user.lastPpt))}* para jugar de nuevo.`);
    if (!options.includes(choice)) return ctx.reply(`⚠️ Usa: *${ctx.usedPrefix ?? "."}ppt piedra|papel|tijera*`);
    const bot = options[Math.floor(Math.random() * options.length)];
    const won = (choice === "piedra" && bot === "tijera") || (choice === "papel" && bot === "piedra") || (choice === "tijera" && bot === "papel");
    const reward = won ? Math.floor(Math.random() * 1401) + 100 : 0;
    user.lastPpt = now;
    user.bolsillo += reward;
    saveEconomy(ctx, ctx.sender, user);
    const result = choice === bot ? "🤝 ¡Empate!" : won ? `🎉 ¡Ganaste! +₡${formatCoins(reward)}` : "❌ ¡Perdiste!";
